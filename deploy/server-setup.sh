...
# Спрашиваем у самого PHP, а не ищем строку в выводе `php -m`. Разбор текста
# через конвейер под `set -o pipefail` оказался ненадёжен: `grep -q` закрывает
# трубу на первом совпадении, php получает SIGPIPE, и вся строка считается
# провалившейся, хотя расширение на месте. Ровно на этом скрипт падал
# на Ubuntu 26.04, показывая pdo_mysql в списке и тут же сообщая, что его нет.
has_ext() { "$PHP_BIN" -r 'exit(extension_loaded($argv[1]) ? 0 : 1);' "$1"; }
