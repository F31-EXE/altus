...
/**
 * Раскладка сетки услуг.
 *
 * Сетка из двенадцати колонок. Пара карточек 5+7 или 6+6 закрывает ряд,
 * одиночная широкая занимает все двенадцать. Поэтому при нечётном числе
 * услуг раздел открывает широкая карточка, а дальше идут пары: ряд всегда
 * заполнен, сколько бы услуг ни завели в админке.
 *
 * @return string[] класс пролёта для каждой услуги, по порядку
 */
function services_spans(int $count): array
{
    $spans = [];
    $left  = max(0, $count);
    ...
    $cycle = [
        ['svc--span-5', 'svc--span-7'],
        ['svc--span-6', 'svc--span-6'],
        ['svc--span-7', 'svc--span-5'],
        ['svc--span-6', 'svc--span-6'],
    ];
    $i = 0;
    while ($left > 0) {
        foreach ($cycle[$i % count($cycle)] as $class) {
            $spans[] = $class;
        }
        $i++;
        $left -= 2;
    ...
    return array_slice($spans, 0, $count);
}

/**
 * Раскладка мозаики «Наших работ».
 *
 * Сетка из шести колонок, и каждый ряд обязан заполняться целиком, иначе
 * в мозаике остаются дыры. Работ в базе может быть сколько угодно, поэтому
 * ряды собираются из готовых шаблонов, каждый из которых даёт в сумме шесть.
 * Хвост из одной или трёх работ закрывается отдельными шаблонами.
 *
 * @return int[] ширина в колонках для каждой работы, по порядку
 */
function works_spans(int $count): array
{
    $cycle = [[4, 2], [2, 4], [2, 4], [3, 3]];
    $spans = [];
    $left  = max(0, $count);
    $i     = 0;
