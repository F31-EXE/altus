...
>   о cookie. Реквизиты оператора — `legal` в конфиге, заполнены и сверены
>   по контрольным суммам. Если любое поле оставить пустым, вместо него
>   встаёт красная пометка и появляется врезка «документ не готов
>   к публикации»: пустой реквизит в правовом документе надо замечать.
