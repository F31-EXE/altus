/* ==========================================================================
   ...
   ОДИН компонент на весь сайт. Его используют:
     - «Наши услуги»  увеличение фотографии услуги и окно с полным описанием
...
(function () {
  ...
  function render() {
    ...
    /* Текст приходит как есть из базы, у описания услуги в нём абзацы.
       Пустая строка - граница абзаца, одиночный перенос остаётся переносом
       внутри абзаца. Разметку не принимаем: экранируем и только потом
       расставляем <br>. */
    if (text) {
      text.split(/\n\s*\n/).forEach(function (para) {
        para = para.trim();
        if (para) html += '<p class="lightbox__text">' + esc(para).replace(/\n/g, '<br>') + '</p>';
      });
    }
